...
class Minimax(Stationary):

    def __init__(self, input_dim, variance=1., lengthscale=None, ARD=False, active_dims=None, name='Minimax'):
        super(Minimax, self).__init__(input_dim, variance, lengthscale, ARD, active_dims, name)

    def to_dict(self):


        input_dict = super(Minimax, self)._save_to_input_dict()
        input_dict["class"] = "GPy.kern.Minimax"
        return input_dict

    @staticmethod
    def _build_from_input_dict(kernel_class, input_dict):
        useGPU = input_dict.pop('useGPU', None)
        return Minimax(**input_dict)

    def K_of_r(self, r):
        
        
        def f(x):
            return (self.variance*(np.sin (np.pi*x))**2/(np.pi*x)**2)
        ...
        for i in range (n):
            for j in range (k):
                if r[i][j] == 0:
                    K[i][j] = float(self.variance + np.random.normal(10**(-10),10**(-11)))
                else:
                    K[i][j] = float(f(r[i][j]))
    ...
    def dK_dr(self, r):
        ...
        def f(x):
            return 2 * np.sin (np.pi*x)* (-np.sin (np.pi*x) + np.pi*x*np.cos (np.pi*x))/(np.pi**2*x**3)
        ...
        for i in range (n):
            for j in range (k):
                if r[i][j] == 0:
                    K[i][j] = float(f(r[i][j]+10**(-10)) +  np.random.normal(10**(-10),10**(-11)))
                else:
                    K[i][j] = float(f(r[i][j]))
